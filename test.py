...
def read_image(path):
    """Read image file, handling both regular images and multi-channel TIFF files."""
    if path.lower().endswith('.tif') or path.lower().endswith('.tiff'):
        # For TIFF files, use tifffile
        img = tifffile.imread(path)
        # If it's a multi-channel image, convert to RGB
        if len(img.shape) == 3 and img.shape[2] > 3:
            img = img[:, :, :3]  # Take first 3 channels
        return img
    else:
        # For regular images, use OpenCV
...
tic = time()
target = 'submits/custom/'
# os.mkdir(target)
for i,name in enumerate(val):
    if i%10 == 0:
        print(f"{i / 10}     {time() - tic:.2f}")
